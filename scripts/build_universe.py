...
def infer_industry(row):
    """Fallback classification when Yahoo screener omits industry metadata."""
...
def score_rows(rows, rules):
    ...
    q = rules["quality"]
    for row in rows:
        mc = num(row.get("marketCap")); vol = num(row.get("averageDailyVolume3Month")); pe = num(row.get("trailingPE"))
        pb = num(row.get("priceToBook")); roe = num(row.get("returnOnEquity")); debt = num(row.get("debtToEquity")); change = num(row.get("regularMarketChangePercent"))
        ...
        raw = (w["market_cap"] * mc_score + w["liquidity"] * liq_score + w["valuation"] * valuation_score + w["financial_health"] * health_score + w["ownership"] * 0.5 + w["trend"] * trend_score + w["data_quality"] * quality_score)
        penalty = 0.0
...
def select_diversified(rows, target, rules):
    ...
    for row in rows:
        ...
        if d.get("require_industry") and industry == "Unknown":
            rejected.append((row, "missing_industry")); continue
        if sector_counts.get(sector, 0) >= max_sector:
            rejected.append((row, "sector_concentration")); continue
        if industry_counts.get(industry, 0) >= max_industry:
            rejected.append((row, "industry_concentration")); continue
        selected.append(row)
        sector_counts[sector] = sector_counts.get(sector, 0) + 1
        industry_counts[industry] = industry_counts.get(industry, 0) + 1
        if len(selected) >= target: break
    if len(selected) < target:
        raise RuntimeError(f"Could only select {len(selected)} of {target} stocks while respecting industry <= {max_industry}, sector <= {max_sector}, and requiring industry data.")
...
def build_market(name, cfg, rules):
    ...
    for row in discovered:
        ...
        if reasons:
            row["rejection_reason"] = ";".join(reasons); rejected.append(row)
    ...
    return {
        ...
        "source": "Yahoo Finance screener via yfinance; normalized/fallback industry groups",
...
def main():
    rules = load_config(); os.makedirs(DATA_DIR, exist_ok=True); os.makedirs(REPORT_DIR, exist_ok=True)
    outputs = {}
